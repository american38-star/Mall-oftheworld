...
export default {
  ...
  methods: {
    ...
    openApproveModal(data, type) {
      ...
      this.showRejectModal = false; // تأكد من إغلاق موذج الرفض
    },
    ...
    // 🔥 دالة للموافقة على السحب مع رسالة (مصححة)
    async approveWithdrawWithMessage(req, message = "") {
      ...
      try {
        // 1. تحديث رصيد المستخدم (خصم المبلغ)
        if (req.userId) {
    ...
    // 🔥 دالة للموافقة على التعبئة مع رسالة (مصححة)
    async approveRechargeWithMessage(r, message = "") {
      ...
      try {
        ...
        console.error("خطأ في الموافقة على التعبئة:", e);
    ...
    openRejectModal(data, type) {
      ...
      this.showModal = false; // إغلاق الموذج القديم
      this.showApproveModal = false; // تأكد من إغلاق موذج الموافقة
    },
    ...
    async rejectWithdraw(req, reason) {
      ...
      try {
        ...
        console.error("خطأ في رفض طلب السحب:", e);
        alert("خطأ في رفض طلب السحب");
      } finally {
        this.processingId = null;
      }
    },
    
    // 🚀 رفض التعبئة مع سبب (مصحح)
    async rejectRecharge(r, reason) {
      ...
      try {
        ...
        console.error("خطأ في رفض طلب التعبئة:", e);
    ...
    viewWithdrawDetails(req) {
      ...
      this.showModal = true;
      this.showApproveModal = false; // إغلاق موذج الموافقة إذا كان مفتوحاً
      this.showRejectModal = false; // إغلاق موذج الرفض إذا كان مفتوحاً
    },
    ...
    // 🔥 دالة جديدة لتحميل سجلات التعبئة (مصححة)
    async loadRechargeLogs() {
      try {
        this.loadingRechargeLogs = true;
        this.rechargeLogs = [];
        
        console.log("🔄 جاري تحميل سجلات التعبئة...");
        ...
        try {
          ...
          const rechargeLogsSnap = await getDocs(rechargeLogsQuery);
          
          const logsFromRechargeLogs = rechargeLogsSnap.docs.map((d) => {
            const data = d.data() || {};
            let createdAt = data.createdAt;
            
            // تحويل Timestamp إلى تاريخ قابل للعرض
          ...
          if (logsFromRechargeLogs.length > 0) {
            this.rechargeLogs = logsFromRechargeLogs;
            console.log(`✅ تم تحميل ${this.rechargeLogs.length} سجل تعبئة من recharge_logs`);
            return;
          }
        } catch (err) {
          console.log("⚠ لا يوجد collection recharge_logs أو خطأ فيه، جارٍ البحث في transactions...", err);
        }
        ...
        try {
          ...
          const logsFromTransactions = transactionsSnap.docs.map((d) => {
            const data = d.data() || {};
            let createdAt = data.createdAt;
          ...
          this.rechargeLogs = logsFromTransactions;
          console.log(`✅ تم تحميل ${this.rechargeLogs.length} سجل تعبئة من transactions`);
          
        } catch (err) {
          console.error("❌ خطأ في تحميل سجلات التعبئة من transactions:", err);
          this.rechargeLogs = [];
        }
